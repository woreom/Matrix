...
class Matrix:
    ...
    def __mul__(self, c): #scalarProd function from left
        C=()
        for row in self.rows:
            r=()
            for i in row:
...
##                s += "\x05\n"
    ...
    def delete_irow_jcolume(self,i,j):
        temp=self.rows[:i]+self.rows[i+1:]
        t=()
        for row in temp:
            row=row[:j]+row[j+1:]
            t+=(row,)
    ...
    def scalarProd1Row(self,c, num_row): ## Row*c -> Row
        rows=[]
        for i in range(self.m):
    ...
    def changeRows(self,num_row1,num_row2): ## Row1 <-> Row2
##        print("e2\t"+str(row1)+"\t"+str(row2))
        rows=[]
        for i in range(self.m):
            rows.append(list(self.rows[i]))
    ...
    def combine(self,c,num_row1, num_row2): ## Row2 += c*Row1
##        print("e3\t"+str(c)+"\t"+str(row1)+"\t"+str(row2))
        rows=[]
        for i in range(self.m):
            rows.append(list(self.rows[i]))
        for i in range(self.n):
    ...
    def organize(self):
        ...
        rows=[]
        for i in range(self.m):
            wayOfReverse.append(["changeRows",[i,a[i][0]]])
...
class SquareMatrix(Matrix):
    ...
    def makeUpTriangle(self):
        ...
        for i in range(self.m):
            for k in range(i+1,self.m):
                a=M.rows[i][i]
                b=M.rows[k][i]
                wayOfReverse.append(["makeUpTriangle",[b*-1.0/a, i, k]])
    ...
    def makeDownTriangle(self):
        ...
        for i in range(M.m):
            for k in range(i):
                    a=M.rows[i][i]
                    b=M.rows[k][i]
                    wayOfReverse.append(["makeDownTriangle",[b*-1.0/a, i, k]])
                    M=M.combine(b*-1.0/a, i,k)
            M=M.organize()
        return M
    ...
    def solve(self):
        ...
        for i in range(M.m):
            wayOfReverse.append(["scalarProd1Row",[1.0/M.rows[i][i],i]])
            M=M.scalarProd1Row(1.0/M.rows[i][i],i)
        return M

    def reverse1(self):
        M=self.solve().clairfy()
        for i in wayOfReverse:
...
##    a=SquareMatrix([[1,2,0,1,0],[6,1,0,2,1],[8,10,5,3,6],[1,2,3,4,3],[1,2,11,23,111]])
    a=SquareMatrix([[-1,0,-2,1],[-2,3,0,0],[1,1,0,1],[0,-1,0,-3]])
##    a=SquareMatrix([[2,6],[3,47]])
    print "The Matrix :"
    print a
    print "reversed without determinant :"
    print a.reverse1().clairfy()
    print "reversed with determinant :"
    print a.reverse().clairfy()
    print "dereminant"
    print a.det()
