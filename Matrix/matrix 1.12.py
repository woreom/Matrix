...
##                s += "\x05\n"
    ...
    def det(self):
        ...
        for i in range(self.n):
            result+=(-1)**i * self.rows[0][i] * self.delete_irow_jcolume(0,i).det()
...
##            result+= (-1)**i * self.rows[0][i] * Matrix(r).det()
    ...
    def changeRows(self, rowsIndex):
        rows=[]
        for i in rowsIndex:
            rows.append(self.rows[i])
        return Matrix(rows)
    
    def change2Rows(self,num_row1,num_row2):
        rows=[]
        for i in range(self.m):
            rows.append(list(self.rows[i]))
    ...
    def organize(self):
        ...
        wayOfEchelon.append(["changeRows",rowsIndex])
        return Matrix(rows)
